...
function ManageSales() {
  ...
  useEffect(() => {
    ...
    if (filterType === "distributor" && filterValue) {
      ...
      results = results.filter(
        (s) => s.product?.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
  ...
  const handleExportCSV = () => {
    ...
    const headers = ["Distributor", "Product", "Quantity", "Total Price", "Total BV", "Date"];
    ...
    <div className="space-y-6 px-4 pb-20 md:px-0 md:pb-0"> 
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            ...
            <p className="text-sm text-gray-500 mt-1">View and filter transaction history.</p>
        </div>
        <button
        ...
        <div className="flex flex-col sm:flex-row gap-2 w-full lg:flex-1">
            ...
            <div className="relative w-full sm:flex-1">
                ...
                {filterType === "distributor" ? (
                    ...
                    <input
                        type="text"
                        placeholder={filterType === "product" ? "Search product name..." : "Filter not active"}
      ...
      <div className="hidden md:block bg-white border border-gray-200 shadow-sm rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
            ...
            <thead className="bg-gray-50/50">
                <tr>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Distributor</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Product</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Price (Total)</th>
                <th className="px-6 py-4 text-center text-xs font-semibold text-gray-500 uppercase tracking-wider">Qty</th>
                <th className="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">BV (Total)</th>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
                {currentItems.length === 0 ? (
                    ...
                    currentItems.map((s) => (
                    <tr key={s._id} className="hover:bg-green-50/30 transition-colors">
                        <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                                <User size={16} className="text-gray-400" />
                                <span className="text-sm font-medium text-gray-900">{s.distributor?.name || "Unknown"}</span>
                            </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                            <div className="flex items-center gap-2">
                                <Package size={16} className="text-gray-400" />
                                <span className="text-sm text-gray-600">{s.product}</span>
                            </div>
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-semibold text-gray-900">{s.totalPrice?.toLocaleString()} KES</span>
                        </td>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                            <span className="text-sm font-medium text-green-700">{s.totalBV}</span>
                        </td>
      ...
      <div className="md:hidden space-y-4">
        {currentItems.length === 0 ? (
            ...
            currentItems.map((s) => (
                <div key={s._id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-200">
                    <div className="flex justify-between items-start mb-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                <p className="text-sm font-bold text-gray-900">{s.distributor?.name || "Unknown"}</p>
                        ...
                        <span className="text-sm font-bold text-gray-900">{s.totalPrice?.toLocaleString()} KES</span>
                    </div>
                    
                    <div className="border-t border-gray-100 pt-3 flex flex-col gap-2">
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500 flex items-center gap-2"><Package size={14}/> Product</span>
                            <span className="font-medium text-gray-700 truncate max-w-[150px]">{s.product}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500 flex items-center gap-2"><layers size={14}/> Quantity</span>
                            <span className="bg-gray-100 text-gray-700 px-2 py-0.5 rounded text-xs font-medium">{s.quantity}</span>
                        </div>
                        <div className="flex justify-between items-center text-sm">
                            <span className="text-gray-500 flex items-center gap-2"><Award size={14}/> BV Points</span>
      ...
      {totalPages > 1 && (
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mt-4">
            ...
            <div className="flex gap-2 order-1 sm:order-2 w-full sm:w-auto justify-center">
                ...
                >
                    <ChevronLeft size={16} /> <span className="ml-1 sm:hidden text-sm">Prev</span>
                </button>
                <button
                    ...
                    className="flex-1 sm:flex-none p-2 rounded-lg border border-gray-300 bg-white text-gray-600 hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed flex justify-center items-center"
                >
                    <span className="mr-1 sm:hidden text-sm">Next</span> <ChevronRight size={16} />
                </button>
            </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
        <div className="bg-white p-4 rounded-xl border border-green-100 shadow-sm flex items-center justify-between">
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs md:text-sm text-gray-400">Based on filtered selection</p>
                </div>
            </div>
            <p className="text-lg md:text-xl font-bold text-gray-900 text-right">{totalP.toLocaleString()} <span className="text-sm font-normal text-gray-500 block md:inline">KES</span></p>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-blue-100 shadow-sm flex items-center justify-between">
            ...
            <p className="text-lg md:text-xl font-bold text-gray-900">{totalBV.toLocaleString()}</p>
        </div>
      </div>
