...
const Modal = ({ open, title, children, onClose }) => {
  if (!open) return null;
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/50 backdrop-blur-sm animate-in fade-in duration-200 p-4 sm:p-0">
...
export default function UserDetails() {
  ...
  if (loading) {
    return (
      <div className="flex flex-col justify-center items-center h-[50vh] text-green-600">
        ...
        <p className="mt-4 text-sm font-medium text-gray-500 animate-pulse">Loading user directory...</p>
      </div>
    );
  }
  ...
  if (error) return (
    <div className="p-8 text-center min-h-[50vh] flex flex-col justify-center items-center">
        ...
        <button onClick={fetchUsers} className="mt-4 px-4 py-2 bg-gray-100 hover:bg-gray-200 rounded-lg text-sm text-gray-700 font-medium transition-colors">Try Again</button>
    </div>
  );

  return (
    <div className="space-y-6 pb-20 md:pb-0"> {/* Extra padding bottom for mobile fab clearance if needed */}
      ...
      <div className="flex flex-col xl:flex-row xl:items-end xl:justify-between gap-4">
        <div>
          ...
          <div className="flex items-center gap-3 mt-1.5 overflow-x-auto">
             <span className="text-sm text-gray-500 whitespace-nowrap">Total: <strong className="text-gray-900">{users.length}</strong></span>
             <span className="w-1 h-1 rounded-full bg-gray-300"></span>
             <span className="text-sm text-gray-500 whitespace-nowrap">Admins: <strong className="text-purple-600">{totalAdmins}</strong></span>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {paginated.length === 0 ? (
            ...
            paginated.map((u) => (
                <div key={u._id} className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col gap-3">
                    <div className="flex justify-between items-start">
                        <div className="flex items-center gap-3">
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold shadow-sm ${u.isAdmin ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>
                        ...
                        <span className={`px-2 py-1 rounded-lg text-[10px] uppercase font-bold tracking-wide border ${u.isAdmin ? "bg-purple-50 text-purple-700 border-purple-100" : "bg-blue-50 text-blue-700 border-blue-100"}`}>
            ...
            <tbody className="divide-y divide-gray-100 bg-white">
                ...
                paginated.map((u) => (
                    ...
                    <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center gap-3">
                            <div className={`w-9 h-9 rounded-full flex items-center justify-center text-sm font-bold shadow-sm ${u.isAdmin ? 'bg-purple-100 text-purple-700' : 'bg-green-100 text-green-700'}`}>
                                {u.username.charAt(0).toUpperCase()}
                            </div>
      ...
      {totalPages > 1 && (
        <div className="bg-white md:bg-gray-50 border md:border-t border-gray-200 md:border-gray-200 rounded-xl md:rounded-none px-6 py-4 flex items-center justify-between shadow-sm md:shadow-none">
            <p className="text-sm text-gray-500">
                Page <span className="font-medium text-gray-900">{currentPage}</span> of <span className="font-medium">{totalPages}</span>
            </p>
      ...
      >
        ...
        >
          ...
          <div className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-200 cursor-pointer hover:bg-gray-100 transition-colors" onClick={() => setFormData({...formData, isAdmin: !formData.isAdmin})}>
            <div className={`w-5 h-5 rounded border flex items-center justify-center transition-colors ${formData.isAdmin ? 'bg-green-600 border-green-600' : 'bg-white border-gray-300'}`}>
                {formData.isAdmin && <User size={12} className="text-white" />}
            </div>
            <div>
                 <p className="text-sm font-medium text-gray-700">Grant Admin Privileges</p>
                 <p className="text-xs text-gray-500">Allows user to manage stock and other users.</p>
            </div>
          </div>
      ...
      >
        ...
        >
          ...
          <div className="md:col-span-2">
            <label className="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5">New Password <span className="normal-case font-normal text-gray-400">(optional)</span></label>
            <input
              type="password"
              value={formData.password}
              onChange={(e) => setFormData({ ...formData, password: e.target.value })}
      ...
      >
        <div className="text-center py-4">
            ...
            <strong className="text-gray-900 text-lg">{selection?.username}</strong>?
            <span className="block text-xs text-red-500 mt-2">This action cannot be undone.</span>
            </p>
